...
```bash
uv run -m lerobot.calibrate \
    --teleop.type=sourccey_v3beta_teleop \
    --teleop.port=COM29 \
    --teleop.id=sourccey_v3beta_teleop
```

To teleoperate, SSH into your Raspberry Pi, navigate to the lerobot repo and run this command:

```
uv run -m lerobot.common.robots.sourccey_v3beta.sourccey_v3beta_host
```
...
```
python examples/sourccey_v3beta/teleoperate.py
```

```
python examples/sourccey_v3beta/record.py
```

```
python examples/sourccey_v3beta/replay.py
```

```
python examples/sourccey_v3beta/evaluate.py
```
