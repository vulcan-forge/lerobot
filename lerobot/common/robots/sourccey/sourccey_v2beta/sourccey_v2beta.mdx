...
```
python -m examples.sourccey.sourccey_v2beta.evaluate --display_data=true
```
...
torchrun --nproc_per_node=2 lerobot/scripts/train.py \
  --dataset.repo_id=local/sourccey_v2beta_001_tape_a  \
  --policy.type=act \
  --output_dir=outputs/train/act_ssourccey_v2beta_001_tape_a \
  --job_name=act_sourccey_v2beta_001_tape_a  \
  --policy.device=cuda \
  --policy.use_amp=true \
  --wandb.enable=false \
