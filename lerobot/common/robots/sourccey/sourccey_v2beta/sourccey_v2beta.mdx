...
```
uv run -m lerobot.common.robots.sourccey.sourccey_v2beta.sourccey_v2beta_host
```
...
```
uv run -m examples.sourccey.sourccey_v2beta.teleoperate --display_data=true
```

```
uv run -m examples.sourccey.sourccey_v2beta.record --display_data=true
```

```
uv run -m examples.sourccey.sourccey_v2beta.replay --display_data=true
```
...
```bash
CUDA_VISIBLE_DEVICES=0 uv run lerobot/scripts/train.py \
 --dataset.repo_id=local/sourccey_v2beta_towel_010_a \
