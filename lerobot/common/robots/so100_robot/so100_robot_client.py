...
from .config_so100_robot import SO100RobotClientConfig, SO100RobotConfig


class SO100RobotClient(Robot):
    config_class = SO100RobotClientConfig
    name = "so100_robot_client"

    def __init__(self, config: SO100RobotClientConfig):
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "SO100RobotClient is already connected. Do not run `robot.connect()` twice."
            )
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("So100ServerClient is not connected. You need to run `robot.connect()`.")
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "So100ServerClient is not connected. You need to run `robot.connect()` before disconnecting."
            )
