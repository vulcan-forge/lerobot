...
python -m lerobot.record \
    ...
    --teleop.left_port=/dev/ttyUSB1 \
    --teleop.right_port=/dev/ttyUSB3 \
    --dataset.repo_id=local/so100_double-001__towel-a__set000__mylesr \
...
python lerobot/scripts/train.py \
  --dataset.repo_id=local/so100_double-001__towel-a__set000__mylesr \
  --policy.type=act \
  --output_dir=outputs/train/act__so100_double-towel-a__set000__mylesr \
  --job_name=act__so100_double-towel-a__set000__mylesr\
  --policy.device=cuda \
  --wandb.enable=false \
...
python -m lerobot.record  \
  ...
  --robot.right_port=/dev/ttyUSB2 \
  --robot.cameras="{ left: {type: opencv, index_or_path: 0, width: 640, height: 480, fps: 30}, right: {type: opencv, index_or_path: 2, width: 640, height: 480, fps: 30}}" \
  --policy.path="outputs/train/act__so100_double-towel-a__set000__mylesr/checkpoints/020000/pretrained_model" \
  --dataset.repo_id=local/eval_so100_double-001__towel-a__set000__mylesr \
